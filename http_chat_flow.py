...
def apply_saved_location_weather_outcome(
    *,
    ...
    saved_location = ""
    if callable(get_saved_location_text):
        try:
            saved_location = str(get_saved_location_text() or "").strip()
        except Exception:
            saved_location = ""
    if explicit_saved_location and (
        not weather_reply
        or "need a confirmed location or coordinates" in weather_reply.lower()
    ):
        weather_reply = (
            f"Got it. Using the saved location from {saved_location}."
            if saved_location
            else "Got it. Using the saved location as specified."
        )
