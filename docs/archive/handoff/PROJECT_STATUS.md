...
## Archive Notice
This file is retained as historical handoff context only.

Archived location after cleanup: `C:\Nova\docs\archive\handoff\PROJECT_STATUS.md`

Do not use this file as the current planning or resume surface.

Use these instead:
- `C:\Nova\This_is_nova`
- `C:\Nova\docs\PHASE_CLOSEOUT_CHECKLIST.md`
- `C:\Nova\docs\BASE_PACKAGE_READINESS.md`
- `C:\Nova\docs\HANDOFF.md`

The content below reflects an older March 2026 project state and is intentionally preserved without being treated as current truth.
...
- Memory precision
  - Good progress, but still needs quality tuning for â€œvital factâ€ recall vs noise in broader conversations.
...
2. Improve `web research` source discovery
   ...
   - Optionally add domain crawl budgets and URL pattern boosts (`student_data`, `attendance`, `tsds`, etc.)
...
- If behavior looks stale, ensure youâ€™re running the latest `nova_core.py` from `C:\Nova` and restart Nova.
