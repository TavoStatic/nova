...
class TestNovaReplyContractsService(unittest.TestCase):
    def test_truthful_limit_reply_uses_request_guidance_for_question(self):
        ...
        self.assertIn("clarifying question", reply.lower())

    def test_attach_learning_invitation_preserves_existing_suffix(self):
        reply = nova_reply_contracts.attach_learning_invitation(
            "I don't know that based on what I can verify right now, and I don't want to make it up. If you know the answer or want to correct me, tell me and I'll store it so I do better next time.",
            truthful_limit=True,
            normalize_turn_text_fn=lambda text: text.lower(),
        )
        self.assertEqual(reply.count("do better next time"), 1)
