...
class TestRuntimeRecovery(unittest.TestCase):
    ...
    def test_powershell_webui_status_uses_logical_processes(self):
        ...
        self.assertIn('Stop-NovaHttpOnPort ([int]$bindPort) | Out-Null', script)
        ...
        self.assertIn('if (-not (Wait-NovaHttpReady $bindHost $bindPort 30)) {', script)
        self.assertIn('Write-Host ("[FAIL] webui process did not become ready on http://" + $bindHost + ":" + $bindPort + "/api/health")', script)
        self.assertIn('Health is the source of truth', script)
        self.assertIn('$r = Invoke-WebRequest -UseBasicParsing $healthUrl -TimeoutSec 8', script)
        self.assertIn('Write-Host ("[FAIL] Requested port " + $bindPort + " is still occupied by non-Nova listener(s): " + $owners)', script)
        self.assertIn('if (-not (Wait-NovaHttpStopped $bindPort 8)) {', script)

        stop_idx = script.index('Stop-NovaHttpOnPort ([int]$bindPort) | Out-Null')
