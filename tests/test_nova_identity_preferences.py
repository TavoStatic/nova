...
class TestNovaIdentityPreferences(unittest.TestCase):
    ...
    def test_is_color_animal_match_question_detects_best_match_prompt(self):
        self.assertTrue(
            nova_identity_preferences.is_color_animal_match_question(
                "What color matches best with the animals I like?"
            )
        )

    def test_extract_last_user_question_keeps_developer_bilingual_lookup(self):
        question = nova_identity_preferences.extract_last_user_question(
            [
                ("user", "is gus bilingual in english and spanish?"),
                ("assistant", "I need more info."),
                ("user", "keep trying"),
            ],
            "keep trying",
            ...
            is_developer_color_lookup_request_fn=lambda text: False,
            is_developer_bilingual_request_fn=lambda text: "bilingual" in text.lower(),
        )

        self.assertEqual(question, "is gus bilingual in english and spanish?")
