...
class TestRuntimeTimelineService(unittest.TestCase):
    def test_payload_combines_operator_guard_and_boot_events(self):
        ...
        self.assertGreaterEqual(payload.get("count"), 3)
    ...
    def test_payload_keeps_guard_failures_when_no_recovery_anchor_exists(self):
        with tempfile.TemporaryDirectory() as td:
            runtime_dir = Path(td) / "runtime"
            log_dir = Path(td) / "logs"
            runtime_dir.mkdir(parents=True, exist_ok=True)
        ...
        self.assertTrue(any(item.get("title") == "Core attempt failed" for item in events))
        self.assertTrue(any(item.get("title") == "Restart backoff armed" for item in events))
