...
from services.nova_identity_answers import assistant_name_reply
...
from services.nova_identity_answers import developer_full_name_reply
from services.nova_identity_answers import self_identity_web_challenge_reply
...
class TestNovaTruthAnswers(unittest.TestCase):
    ...
    def test_scan_request_is_refused(self):
        ...
        self.assertIn("scan your machine", reply)


class TestNovaIdentityReplyHelpers(unittest.TestCase):
    def test_self_identity_web_challenge_reply_mentions_verified_name(self):
        reply = self_identity_web_challenge_reply(assistant_name="Nova")
        self.assertIn("should not need web research", reply)
        self.assertIn("my name is Nova", reply)

    def test_assistant_name_reply_handles_confirmation(self):
        self.assertEqual(
            assistant_name_reply("your name is nova", assistant_name="Nova"),
            "Correct. My name is Nova.",
        )

    def test_developer_full_name_reply_mentions_nickname_when_distinct(self):
        reply = developer_full_name_reply(
            developer_name="Gustavo Uribe",
            developer_nickname="Gus",
        )
        self.assertIn("Gustavo Uribe", reply)
        self.assertIn("Gus is his nickname", reply)
