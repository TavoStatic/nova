...
class TestHttpChatFlow(unittest.TestCase):
    ...
    def test_apply_saved_location_weather_outcome_handles_explicit_saved_location_without_state(self):
        class _Session:
            def __init__(self):
                self.conversation_state = {"kind": "before"}
        ...
        out = http_chat_flow.apply_saved_location_weather_outcome(
            conversation_state={"kind": "other"},
            routed_text="yes please use the saved location",
            weather_for_saved_location=lambda: "",
            is_saved_location_weather_query=lambda text: True,
            get_saved_location_text=lambda: "integration-test-memory: bravo-98765",
            session=session,
            ledger={},
            make_conversation_state=lambda kind: {"kind": kind},
            action_ledger_add_step=lambda *a, **k: None,
        ...
        self.assertTrue(out.get("handled"))
        self.assertEqual(out.get("reply"), "Got it. Using the saved location from integration-test-memory: bravo-98765.")

    def test_apply_saved_location_weather_outcome_handles_explicit_saved_location_without_label(self):
        class _Session:
            def __init__(self):
                self.conversation_state = {"kind": "before"}
        ...
        self.assertTrue(out.get("handled"))
        self.assertEqual(out.get("reply"), "Got it. Using the saved location as specified.")
