...
def _autonomy_maintenance_summary() -> dict:
    ...
    if isinstance(stale_value, bool):
        ...
        last_regression_stale = bool(
            ...
            and str(payload.get("last_regression_date") or "") != time.strftime("%Y-%m-%d")
        )
...
def _work_trees_payload(limit: int = 32) -> dict:
    ...
    if limit is not None:
        try:
            all_trees = work_tree.list_visual_trees(None)
        except Exception:
            all_trees = []
        ...
        for tree_payload in list(all_trees or []):
            if not _priority_tree(tree_payload):
                continue
...
def _generate_chat_reply(
    ...
    skip_action_planner = bool(re.match(r"^\s*explain\b", text or "", flags=re.I))

    if skip_action_planner:
        actions = []
    else:
        try:
    ...
    try:
        ...
        try:
            flow_result = http_chat_flow.apply_saved_location_weather_outcome(
                ...
                get_saved_location_text=nova_core.get_saved_location_text,
                session=session,
                ledger=ledger,
                make_conversation_state=nova_core._make_conversation_state,
                action_ledger_add_step=nova_core.action_ledger_add_step,
    ...
    def do_GET(self) -> None:
        ...
        if path == "/control":
            ...
            if not ok_page:
                if reason_page == "control_login_required":
                    ...
                    _json_response(self, 403, {"ok": False, "error": reason_page})
                return
            _text_response(self, 200, _render_control_html())
            return
        ...
        if path == "/api/control/work-trees":
            ...
            if not ok:
                _json_response(self, 403, {"ok": False, "error": reason})
                return
            _json_response(self, 200, _work_trees_payload())
            return
