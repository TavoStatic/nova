...
def color_animal_match_reply(
    ...
    *,
    ...
    extract_animal_preferences_fn: Callable[[list[tuple[str, str]]], list[str]],
    extract_animal_preferences_from_memory_fn: Callable[[], list[str]],
    pick_color_for_animals_fn: Callable[[list[str], list[str]], str],
) -> str:
    colors = extract_color_preferences_fn(turns)
    if not colors:
    ...
    if not colors:
        return "I can't pick a best color yet because I don't have your color preferences."
    if not animals:
        return "I can't pick a best color for animals yet because I don't have your animal preferences."

    best = str(pick_color_for_animals_fn(colors, animals) or "").strip()
    if not best:
        return "I can't pick a best color yet because I don't have enough preference detail."

    reply = f"Direct answer: {best} matches best with the animals you like ({', '.join(animals)})."
    if len(colors) > 1:
        reply += f" I considered your options: {', '.join(colors)}."
    return reply
