...
class RuntimeTimelineService:
    ...
    @staticmethod
    def _guard_recovery_anchor_ts(events: list[dict]) -> int:
        anchor_actions = {
            "guard_online",
            "guard_adopted_running_core",
            "core_reached_running_state",
            "boot_success",
        }
        anchor_ts = 0
        for event in list(events or []):
            if str(event.get("source") or "").strip().lower() != "guard":
                continue
            if str(event.get("service") or "").strip().lower() not in {"guard", "core"}:
                continue
            if str(event.get("action") or "").strip().lower() not in anchor_actions:
                continue
            try:
                anchor_ts = max(anchor_ts, int(event.get("ts") or 0))
            except Exception:
                continue
        return anchor_ts

    def prune_stale_guard_recovery_noise(self, events: list[dict]) -> list[dict]:
        anchor_ts = self._guard_recovery_anchor_ts(events)
        if anchor_ts <= 0:
    ...
    def payload(self, *, limit: int = 24, control_audit_log, guard_log_path, boot_history_path, safe_tail_lines_fn, time_module) -> dict:
        ...
        for event in events:
            ...
            unique[key] = event
        filtered = self.prune_stale_guard_recovery_noise(list(unique.values()))
        ordered = sorted(filtered, key=lambda item: (int(item.get("ts") or 0), str(item.get("title") or "")), reverse=True)
        return {
