        ...
        if isinstance(value, dict):
            ...
            for sub_key, sub_value in list(value.items())[:8]:
                if isinstance(sub_value, (int, float, bool)):
                    items[str(sub_key)] = sub_value
                elif isinstance(sub_value, str) and sub_value.strip():
                    items[str(sub_key)] = sub_value[:120]
            if items:
                clean_data[str(key)] = items
...
def action_ledger_route_summary(record_or_trace: object | None) -> str:
...
def action_history_reply(
    ...
    *,
    ...
    rec = latest_action_ledger_record(action_ledger_dir)
    if not rec:
