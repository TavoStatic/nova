...
def parse_scoped_patch_payload(value: object) -> dict[str, object]:
    ...
    if not isinstance(target, dict):
        return {}
    if str(candidate.get("scope") or "").strip().lower() != "single_block_only":
        return {}
    return dict(candidate)


def execute_scoped_patch_payload(payload: dict[str, object], *, base_dir: Path) -> dict[str, object]:
    ...
    if not file_name or not function_name or not block_name:
        return {"ok": False, "scope_ok": False, "verified": False, "reason": "missing_target_fields"}
    if not isinstance(start_line, int) or not isinstance(end_line, int):
        return {"ok": False, "scope_ok": False, "verified": False, "reason": "invalid_target_lines"}
    if start_line <= 0 or end_line < start_line:
        return {"ok": False, "scope_ok": False, "verified": False, "reason": "invalid_target_range"}

    root = Path(base_dir).resolve()
    target_path = (root / file_name).resolve()
    try:
        target_path.relative_to(root)
    except Exception:
        return {"ok": False, "scope_ok": False, "verified": False, "reason": "scope_expanded"}
    if not target_path.exists() or not target_path.is_file():
        return {"ok": False, "scope_ok": False, "verified": False, "reason": "target_not_found"}

    try:
        lines = target_path.read_text(encoding="utf-8").splitlines()
    except Exception as exc:
        return {"ok": False, "scope_ok": False, "verified": False, "reason": f"target_read_failed:{exc}"}
    if end_line > len(lines):
        return {"ok": False, "scope_ok": False, "verified": False, "reason": "target_range_out_of_bounds"}

    block_text = "\n".join(lines[start_line - 1:end_line])
    if not block_text.strip():
        return {"ok": False, "scope_ok": True, "verified": False, "reason": "empty_target_block"}

    verification = dict(payload.get("verification") or {}) if isinstance(payload.get("verification"), dict) else {}
    required_fragments = [
        str(item).strip()
        for item in list(verification.get("required_fragments") or verification.get("contains") or [])
        if str(item).strip()
    ]
    forbidden_fragments = [
        str(item).strip()
        for item in list(verification.get("forbidden_fragments") or verification.get("not_contains") or [])
        if str(item).strip()
    ]
    missing = [fragment for fragment in required_fragments if fragment not in block_text]
    forbidden_present = [fragment for fragment in forbidden_fragments if fragment in block_text]
    if missing:
        return {
            "ok": False,
            "scope_ok": True,
            "verified": False,
            "reason": "missing_required_fragments",
            "missing": missing,
    ...
    if forbidden_present:
        return {
            ...
            "scope_ok": True,
            "verified": False,
            "reason": "forbidden_fragments_present",
    ...
    return {
        ...
        "scope_ok": True,
        "verified": True,
        "message": "scoped patch payload verified",
        "verified_target": dict(target),
        "task_id": str(payload.get("task_id") or "").strip(),
...
def control_status_patch_fields(*, patch_summary: dict, patch_action_readiness: dict) -> dict:
    summary = patch_summary if isinstance(patch_summary, dict) else {}
    return {
        "patch_status_ok": bool(summary.get("ok", False)),
        "patch_enabled": bool(summary.get("enabled", False)),
        "patch_strict_manifest": bool(summary.get("strict_manifest", False)),
        "patch_allow_force": bool(summary.get("allow_force", False)),
        "patch_behavioral_check": bool(summary.get("behavioral_check", False)),
        "patch_behavioral_check_timeout_sec": int(summary.get("behavioral_check_timeout_sec", 0) or 0),
        "patch_tests_available": bool(summary.get("tests_available", False)),
        "patch_pipeline_ready": bool(summary.get("pipeline_ready", False)),
        "patch_ready_for_validated_apply": bool(summary.get("ready_for_validated_apply", False)),
        "patch_current_revision": int(summary.get("current_revision", 0) or 0),
        "patch_previews_total": int(summary.get("previews_total", 0) or 0),
        "patch_previews_pending": int(summary.get("previews_pending", 0) or 0),
        "patch_previews_approved": int(summary.get("previews_approved", 0) or 0),
        "patch_previews_rejected": int(summary.get("previews_rejected", 0) or 0),
        "patch_previews_eligible": int(summary.get("previews_eligible", 0) or 0),
        "patch_previews_approved_eligible": int(summary.get("previews_approved_eligible", 0) or 0),
        "patch_previews_orphaned": int(summary.get("previews_orphaned", 0) or 0),
        "patch_review_previews_total": int(summary.get("review_previews_total", 0) or 0),
        "patch_review_previews_pending_distinct": int(summary.get("review_previews_pending_distinct", 0) or 0),
        "patch_review_previews_pending_superseded": int(summary.get("review_previews_pending_superseded", 0) or 0),
        "patch_review_previews_approved_distinct": int(summary.get("review_previews_approved_distinct", 0) or 0),
        "patch_review_previews_approved_superseded": int(summary.get("review_previews_approved_superseded", 0) or 0),
        "patch_review_previews_orphaned": int(summary.get("review_previews_orphaned", 0) or 0),
        "patch_review_previews_superseded_total": int(summary.get("review_previews_superseded_total", 0) or 0),
        "patch_last_preview_name": str(summary.get("last_preview_name") or ""),
        "patch_last_preview_status": str(summary.get("last_preview_status") or ""),
        "patch_last_preview_decision": str(summary.get("last_preview_decision") or ""),
        "patch_last_log_line": str(summary.get("last_patch_log_line") or ""),
        "patch_previews": list(summary.get("previews") or []) if isinstance(summary.get("previews"), list) else [],
...
def archive_preview_report(path_or_name: str, *, updates_dir: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            return {"ok": False, "error": str(exc), "source": str(path), "archived": str(target)}
...
def patch_apply(
    ...
    *,
    ...
    scoped_payload = parse_scoped_patch_payload(zip_path)
    if scoped_payload:
        return execute_scoped_patch_payload(scoped_payload, base_dir=base_dir)
...
def patch_preview(
    ...
    *,
    ...
    tmp_root = updates_dir / "_tmp_patch_preview"
    tmp_root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(dir=str(tmp_root), ignore_cleanup_errors=True) as tmp_dir:
...
def teach_propose_patch(
    ...
    *,
    ...
    interactive_patch_review_enabled_fn: Callable[[], bool],
    interactive_preview_review_fn: Callable[[str], str],
) -> str:
    try:
        teach_dir = updates_dir / "teaching"
        examples_file = teach_dir / "examples.jsonl"
        ...
        with zipfile.ZipFile(out_zip, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            archive.write(examples_file, arcname="examples.jsonl")
            archive.write(tmp_manifest, arcname=patch_manifest_name)
        ...
        try:
            preview_out = patch_preview_fn(str(out_zip), write_report=True)
        except Exception:
        ...
        try:
            ...
            if (
                interactive_patch_review_enabled_fn()
                and sys.stdin
                and hasattr(sys.stdin, "isatty")
                and sys.stdin.isatty()
            ):
                match = re.search(r"Preview written:\s*(.+)$", preview_out or "", flags=re.M)
                preview_path = match.group(1).strip() if match else None
                if preview_path:
                    decision_msg = interactive_preview_review_fn(preview_path)
                else:
        ...
        return f"Failed to create teach proposal: {exc}"


def teach_autoapply_proposal(
    zip_path: str,
    ...
    *,
    ...
    try:
        proposal_zip = Path(zip_path)
        if not proposal_zip.exists():
            return f"Not found: {proposal_zip}"

        try:
            preview_out = patch_preview_fn(str(proposal_zip), write_report=True)
        except Exception as exc:
            try:
                previews = updates_dir / "previews"
                previews.mkdir(parents=True, exist_ok=True)
                tsf = time.strftime("%Y%m%d_%H%M%S")
                fail_fn = previews / f"preview_fail_{tsf}_{proposal_zip.name}.txt"
                fail_fn.write_text(f"Preview generation failed: {exc}", encoding="utf-8")
            except Exception:
                pass
        ...
        with zipfile.ZipFile(proposal_zip, "r") as archive:
