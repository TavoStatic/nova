...
from services.nova_profile_followups import color_reply as service_color_reply
    ...
    while True:
        ...
        try:
            saved_location_weather_outcome = apply_saved_location_weather_outcome(
                ...
                get_saved_location_text=core.get_saved_location_text,
                session=session_state,
                ledger={},
                make_conversation_state=core._make_conversation_state,
                action_ledger_add_step=lambda _ledger, stage, outcome, detail="", **data: _trace(stage, outcome, detail, **data),
        ...
        sequence_reply, sequence_meta = service_execute_cli_sequence(
            ...
            is_student_data_attendance_rules_query=getattr(core, "_is_student_data_attendance_rules_query", lambda _text: False),
            student_data_attendance_rules_reply=getattr(core, "_student_data_attendance_rules_reply", lambda: ""),
            ...
            build_grounded_answer=lambda query, max_sources=2: getattr(core, "_build_grounded_answer", lambda _query, max_sources=2: "")(
                query,
            ...
            developer_color_reply=lambda turns: service_developer_color_reply(
                turns,
                extract_developer_color_preferences_fn=core._extract_developer_color_preferences,
                extract_developer_color_preferences_from_memory_fn=core._extract_developer_color_preferences_from_memory,
                prefix_from_earlier_memory_fn=core._prefix_from_earlier_memory,
            ),
            developer_bilingual_reply=lambda turns: service_developer_bilingual_reply(
                turns,
                developer_is_bilingual_fn=core._developer_is_bilingual,
                developer_is_bilingual_from_memory_fn=core._developer_is_bilingual_from_memory,
                prefix_from_earlier_memory_fn=core._prefix_from_earlier_memory,
            ),
            color_reply=lambda turns: service_color_reply(
                turns,
                extract_color_preferences_fn=core._extract_color_preferences,
                extract_color_preferences_from_memory_fn=core._extract_color_preferences_from_memory,
                prefix_from_earlier_memory_fn=core._prefix_from_earlier_memory,
            ),
            animal_reply=lambda turns: service_animal_reply(
                turns,
                extract_animal_preferences_fn=core._extract_animal_preferences,
                extract_animal_preferences_from_memory_fn=core._extract_animal_preferences_from_memory,
                prefix_from_earlier_memory_fn=core._prefix_from_earlier_memory,
            ),
        ...
        if core._is_color_animal_match_question(routed_user_text):
            msg = service_color_animal_match_reply(
                session_turns,
                extract_color_preferences_fn=core._extract_color_preferences,
                extract_color_preferences_from_memory_fn=core._extract_color_preferences_from_memory,
                extract_animal_preferences_fn=core._extract_animal_preferences,
                extract_animal_preferences_from_memory_fn=core._extract_animal_preferences_from_memory,
                pick_color_for_animals_fn=core._pick_color_for_animals,
            )
            print(f"Nova: {msg}\n", flush=True)
