...
class ConversationSession:
    ...
    def set_language_mix_spanish_pct(self, value: int) -> None:
        try:
            parsed = int(value)
        except (TypeError, ValueError):
            parsed = 0
        self.language_mix_spanish_pct = max(0, min(100, parsed))
    ...
    def reflection_summary(self) -> dict:
        ...
        if int(self.language_mix_spanish_pct or 0) > 0:
            overrides.append("language_mix_spanish_pct")
        summary = {
            "active_subject": self.active_subject(),
            "continuation_used": self.continuation_used_last_turn,
